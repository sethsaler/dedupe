...
| ID | P | Device | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SHEET-01 | P1 | mouse | The preview counts down from 10:00 and shows counts, bytes, and any review-quarantine split ([Start](../ui/action-sheet.md#start)). | A selection including a low-res or random-review file. | 1. Press **Delete All Selected Exact Matches** (or `a`).<br>2. Read the sheet. | The split note leads the sheet when it applies; totals + "Verified against the current selection · preview valid for 10:00" counting down. | — |
