...
| ID | P | Device | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NONHUMAN-01 | P1 | mouse | Nothing arrives selected; per-candidate Trash is one click ([While extended](../ui/no-person-review.md#while-extended)). | A scan with no-person enabled over people-free images. | 1. Open the Non-Human tab.<br>2. Press a candidate's Trash button. | Nothing selected on arrival; the file moves to Trash with no confirmation sheet; a toast offers Undo. | — |
