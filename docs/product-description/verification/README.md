...
**Scripted + automated + e2e pass, 2026-09-03**, against the post-improvement working tree (through commit `8fee7e7`). Three scripted CLI runs (scratch fixtures, scratch `HOME` except one real-Trash round-trip that was restored and cleaned up) covered every scriptable cli.md row and SCANP-01/05/06/09 + SESSION-01 in foundations.md. Eleven new browser tests (`test_empty_path_scan_is_rejected`, `test_attention_navigation_and_space_u_shortcuts`, `test_lightbox_enter_wrap_focus_trap_and_esc`, `test_stale_preview_re_previews_on_confirm`, `test_escape_discards_preview_and_reopening_renews_the_token`, `test_parallel_streams_toggle_controls_cross_folder_groups`, `test_resume_banner_reports_pruned_files_and_discard_starts_clean`, `test_selections_survive_a_server_restart`, `test_sticky_toast_queues_newer_toasts`, `test_random_review_decision_mechanics`, `test_scan_streams_groups_and_cancel_restores_previous`) plus four new API-level tests covered most remaining ui.md and foundations.md rows; many more rows recorded `pass (automated)` against existing unit tests. The pass found and fixed four real defects — B-07 (busy-port crash), B-08 (silent isolate cancel), B-09 (parallel-cancel crash + cancel deadlock; cancelled scans now persist completed cache work), B-10 (focus-trap escape) — see [bug-triage.md](../bug-triage.md). Sixteen documents moved to `verified`: all of foundations/, cli/ except ui-command, cross-cutting/, and ui/{lightbox, low-res-review, random-review, no-person-review, all-files-review}.

**Hand rows still open.** What remains needs a person at the machine: SCANSET-01 (native picker), SCANSET-03 (watch the progress panel), SCANSET-05 (locked selections during a scan), SCANSET-06 (localStorage persistence), SCANSET-08 (reload re-attach), SCANSET-10 (an OpenCV-less environment), GROUPS-10 (OS light mode), GROUPS-11 (suggested-selection framing flip), GROUPS-12 (focused group across reload), GROUPS-13 (sidebar window sliding with hundreds of groups), SHEET-06 (action-bar narration timing), FACES-01 badges and FACES-02 (filter positions — needs real face photos), RESUME-03 through RESUME-07 (banner dismissal, rescan button, one-time toasts), and the browser auto-open half of UICMD-01. Suggested session: scan a real-but-disposable photo folder, walk these in checklist order, and have a second browser tab ready for SCANSET-08/SHEET flows.
