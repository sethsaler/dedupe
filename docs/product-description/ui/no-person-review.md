...
Detection runs during the scan only when the no-person review is enabled. Images, up to 16 GIF frames, and up to 16 directly-seeked video frames are analyzed, with early exit the moment a person is found. The backend is chosen at scan time in Scan setup: `opencv` (default; the bundled YuNet face model runs first at a lower presence threshold, including a mirrored pass and tiled crops, then INRIA and Daimler full-body detectors), `photon` (opt-in; Moondream through the local Photon runtime, roughly a 10 GB model download on first use, querying woman / girl / person / face), or `ensemble` (OpenCV positives first, then Photon on uncertain frames). Photon and ensemble no-person piles get a YuNet + genderage second look. The detectors report person/face boxes, not a calibrated confidence score, so the UI shows frame counts rather than scores.

The photo checks retain extra detail for small or distant faces before examining overlapping regions. Short animations with up to 16 frames are checked frame by frame; longer animations are sampled.

Only *current* no-person decisions populate the category. A file with a counted face — especially a female face — is kept out even if the person detector said no. The membership rule is applied again whenever results are loaded: a file whose verdict is positive, missing, stale, or face-protected — recorded under a detector or model configuration that no longer matches — never appears in the flow. Such files are simply re-analyzed on the next scan. Candidates are ordered newest first by modification time. Nothing is selected, nothing is reviewed, and there is no suggested keeper — the independent-candidates policy. The list pages at 50 cards per page.
