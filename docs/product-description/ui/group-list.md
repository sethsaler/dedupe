...
The list loads whatever the current result holds — a fresh scan streams groups in as they are found, a completed scan shows them all, a resumed session shows its pruned survivors. The sidebar tab defaults to All. Each group card shows its kind, member count, reclaimable size, and state glyph (● / ✔). The first group becomes the focused group; focusing a group loads its members into the detail pane (paginated, 50 member cards per page with a page control and a count summary). The Faces and Files categories also get a sort control on the pager: face count or newest for Faces; path, largest, newest, or oldest for Files — the choice is remembered per category for the session, and the [lightbox](lightbox.md) steps in the same order. Files additionally gets a modified-time filter beside the sort control (last hour, day, week, or month), which narrows the shown cards instantly; see [The all-files review](all-files-review.md).
