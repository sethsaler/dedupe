...
- **Distinct pairs** — a similar group marked *Mark as distinct* records its files as pairwise not-duplicates, and grouping refuses to reunite them while the entries are current. Each entry stores both the stat identity and a perceptual content identity (image hashes / video fingerprint) per file: the pair stays suppressed after a rename or a metadata-only change, and only a real content change — different perceptual hashes — revives it.
